...
export function AIInsights() {
  ...
  const { data: stocks } = useCollection(stocksRef);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const context = {
        inventory: stocks?.map(s => ({ name: s.itemName, stock: s.currentStock, min: s.minStock })),
        accounting: invoices?.filter(i => i.status !== 'paid').map(i => ({ no: i.invoiceNumber, amount: i.totalAmount, date: i.createdAt })),
        laundry: laundry?.filter(l => l.status !== 'paid').map(l => ({ room: l.roomNumber, status: l.status, hotelTotal: l.hotelTotal })),
        maintenance: tasks?.filter(t => t.status !== 'completed').map(t => ({ area: t.roomId, type: t.taskType, priority: t.priority })),
        rooms: rooms?.map(r => ({ no: r.roomNumber, status: r.status, updated: r.updatedAt }))
      };
  ...
  useEffect(() => {
    if (stocks && invoices && laundry && tasks && rooms && !insights) {
      runAnalysis();
    }
  }, [stocks, invoices, laundry, tasks, rooms]);

  if (!insights && !isAnalyzing) return null;
